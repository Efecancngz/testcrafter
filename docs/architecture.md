...
**Deliberate exception:** the test suite still uses `Base.metadata.create_all()`, but only against isolated, temporary SQLite databases created for the duration of a single test (e.g. the `db_session` fixture in `backend/tests/conftest.py`, and the isolated engine `test_list_projects_without_session_override_does_not_500` in `backend/tests/test_api_projects.py` points `SessionLocal` at via `monkeypatch`). This isn't an inconsistency — tests need a fast, fully-isolated schema per run, and `create_all` reflects the *models* directly, which is exactly what a test needs. No test creates the schema on the app's real default database (`sqlite:///./testcrafter.db`); doing so would leave behind a "stale pre-Alembic" file with no `alembic_version` row, which is exactly the shape of database this section's migration workflow (and `CONTRIBUTING.md`) warns against. Production and dev databases are the only ones migrations apply to.
