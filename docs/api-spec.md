...
For each scenario, a `Run` row is written (`status = "passed"` only if every step passed, otherwise `"failed"`) along with one `RunStep` row per executed step (status + log message). Each executed step's `screenshot_path` is a `/screenshots/{run_id}/{step_index}.png` URL, served by a `StaticFiles` mount over `backend/data/screenshots/` (see `app.main`); it's `None` only if the screenshot capture call itself failed, independent of the step's pass/fail outcome. Returns the list of created runs with their steps. 404s if the scan doesn't exist, same pattern as `GET /scans/{scan_id}`.
